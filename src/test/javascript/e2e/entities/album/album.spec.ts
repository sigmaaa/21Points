...
describe('Album e2e test', () => {
  ...
  it('should load Albums', async () => {
    ...
    expect(await albumComponentsPage.getTitle()).to.eq('Albums');
  });

  it('should load create Album page', async () => {
    await albumComponentsPage.clickOnCreateButton();
    albumUpdatePage = new AlbumUpdatePage();
    expect(await albumUpdatePage.getPageTitle()).to.eq('Create or edit a Album');
  ...
  it('should delete last Album', async () => {
    ...
    expect(await albumDeleteDialog.getDialogTitle()).to.eq('Are you sure you want to delete this Album?');
